...
// sub-band), the average-trait drift (diet, forage and hunt, the warmth/wetness
// climate preferences, plus speed/size normalised to their gene ranges so they
// share a 0–1 axis), the speciation count (distinct
...
export class Charts {
  ...
  draw(history) {
    ...
    this.panel(panelH + GAP, panelH, "Avg traits", "1.0", [
      ...
      { color: "#f0a36f", label: "warm", values: s.map((d) => d.warmthPref ?? 0.5) },
      { color: "#6fd3e0", label: "wet", values: s.map((d) => d.wetnessPref ?? 0.5) },
