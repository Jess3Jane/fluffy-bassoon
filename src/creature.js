...
export class Creature {
  ...
  update(dt, world, rng) {
    ...
    // plus the local spatial offset at its position, clamped back onto [0, 1].
    // That offset is the static microclimate *plus the living vegetation feedback*
    // (`world.warmthOffsetAt` / `wetnessOffsetAt`), so a creature standing in a
    // dense single-kind stand feels that stand's nudge toward its kind's climate —
    // the larder shaping selection, not just the seed. So the same instant is warm
    // in one corner of the map and cool in another, and `warmthPref` /
    // `wetnessPref` partition the population across space as well as time. Pure in
    // position, sim-time, and the (serialized) larder — no rng — so it leaves the
    // deterministic stream untouched and replays bit-identically.
    const warmth = clamp01(climateWarmth(world.time) + world.warmthOffsetAt(this.x, this.y));
    const wetness = clamp01(climateWetness(world.time) + world.wetnessOffsetAt(this.x, this.y));
